...
final class PaddedToggleButton: NSButton {
    var contentPadding = NSEdgeInsets(top: 5, left: 9, bottom: 5, right: 9) {
        didSet {
...
final class InlineIconToggleButton: NSButton {
    ...
    func setTitleFontSize(_ size: CGFloat) {
        ...
        self.titleField.cell?.font = self.titleField.font
    }
    ...
    private func configure() {
        ...
        self.titleField.font = NSFont.systemFont(ofSize: NSFont.smallSystemFontSize - 1, weight: .medium)
        self.titleField.alignment = .center
        self.titleField.lineBreakMode = .byClipping
        self.titleField.cell?.lineBreakMode = .byClipping
        self.titleField.cell?.truncatesLastVisibleLine = false
        ...
        self.stack.spacing = 3
        self.stack.translatesAutoresizingMaskIntoConstraints = false
        self.stack.addArrangedSubview(self.iconView)
        self.stack.addArrangedSubview(self.titleField)
        self.addSubview(self.stack)

        let iconWidth = self.iconView.widthAnchor.constraint(equalToConstant: 14)
        let iconHeight = self.iconView.heightAnchor.constraint(equalToConstant: 14)
...
final class StackedToggleButton: NSButton {
    ...
    var contentPadding = NSEdgeInsets(top: 4, left: 5, bottom: 4, right: 5) {
        didSet {
            self.paddingConstraints.first { $0.firstAttribute == .top }?.constant = self.contentPadding.top
            self.paddingConstraints.first { $0.firstAttribute == .leading }?.constant = self.contentPadding.left
            self.paddingConstraints.first { $0.firstAttribute == .trailing }?.constant = -self.contentPadding.right
    ...
    private func configure() {
        ...
        self.iconView.imageScaling = .scaleNone
        self.iconView.translatesAutoresizingMaskIntoConstraints = false
        self.titleField.font = NSFont.systemFont(ofSize: NSFont.smallSystemFontSize - 2, weight: .medium)
        self.titleField.alignment = .center
