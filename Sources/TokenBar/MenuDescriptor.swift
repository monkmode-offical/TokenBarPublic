...
struct MenuDescriptor {
    ...
    private static func metaSection(updateReady: Bool) -> Section {
        var entries: [Entry] = []
        if updateReady {
            entries.append(.action("New update available - Update now", .installUpdate))
        }
        entries.append(contentsOf: [
            .action("Open TokenBar App", .openApp),
...
extension MenuDescriptor.MenuAction {
    var systemImageName: String? {
        switch self {
        case .installUpdate: MenuDescriptor.MenuActionSystemImage.refresh.rawValue
        case .openApp: MenuDescriptor.MenuActionSystemImage.openApp.rawValue
