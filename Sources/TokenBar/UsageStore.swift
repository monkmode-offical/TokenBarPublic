...
extension UsageStore {
    ...
    func observeSettingsChanges() {
        withObservationTracking {
            ...
            _ = self.settings.usageBudgetModeEnabled
            _ = self.settings.usageBudgetTargetDays
            _ = self.settings.usageAlertNotificationsEnabled
            _ = self.settings.usageAlertThresholdsDisplay
            _ = self.settings.sessionTrackingModeEnabled
            _ = self.settings.overviewPrioritySortMode
            ...
            _ = self.settings.licenseState
            _ = self.settings.licenseStatusMessage
...
final class UsageStore {
    ...
    @ObservationIgnored let telemetryStore: UsageTelemetryStore
    ...
    @ObservationIgnored var lastKnownPrimaryResetAt: [UsageProvider: Date] = [:]
    @ObservationIgnored var lastKnownUsageAlertThreshold: [UsageProvider: Int] = [:]
    @ObservationIgnored private var dockBadgeClearTask: Task<Void, Never>?
    ...
    init(
        ...
        telemetryStore: UsageTelemetryStore = UsageTelemetryStore())
    {
    ...
    func sourceContextHint(for provider: UsageProvider) -> String? {
        let sourceLabel = self.sourceLabel(for: provider)
        let attempts = self.fetchAttempts(for: provider)
        let successfulAttempt = attempts.last { attempt in
            attempt.wasAvailable && (attempt.errorDescription?.isEmpty ?? true)
        }

        let channel: String = if let successfulAttempt {
            Self.sourceChannelLabel(for: successfulAttempt.kind)
        } else {
            Self.sourceChannelLabel(from: sourceLabel)
        }
        guard !channel.isEmpty else { return nil }

        let cleanedSource = sourceLabel.trimmingCharacters(in: .whitespacesAndNewlines)
        if cleanedSource.isEmpty || cleanedSource == "auto" {
            return "Source: \(channel)"
        }
        if cleanedSource.caseInsensitiveCompare(channel) == .orderedSame {
            return "Source: \(channel)"
        }
        return "Source: \(channel) (\(cleanedSource))"
    }

    private static func sourceChannelLabel(for kind: ProviderFetchKind) -> String {
        switch kind {
        case .cli:
            "Terminal"
    ...
    private static func sourceChannelLabel(from sourceLabel: String) -> String {
        let lower = sourceLabel.lowercased()
        ...
        if lower.contains("api") || lower.contains("token") {
            return "API token"
        }
    ...
    func refresh(forceTokenUsage: Bool = false) async {
        ...
        if !self.settings.isLicenseUnlocked {
            ...
            guard self.settings.isLicenseUnlocked else {
                self.applyLicenseLockState()
                return
            }
        }
    ...
    private func applyLicenseLockState() {
        let message = self.settings.licenseStatusMessage.isEmpty
            ? "License required. Open Settings > General > License."
            : self.settings.licenseStatusMessage
        for provider in UsageProvider.allCases {
            ...
            self.tokenErrors.removeValue(forKey: provider)
            self.statuses.removeValue(forKey: provider)
            self.probeLogs.removeValue(forKey: provider)
        }
        self.credits = nil
        ...
        self.persistWidgetSnapshot(reason: "license-lock")
    }
    ...
    private func usageAlertWindow(provider: UsageProvider, snapshot: UsageSnapshot) -> RateWindow? {
        if let primary = snapshot.primary {
            return primary
        }
        if provider == .copilot {
    ...
    private func highestCrossedUsageThreshold(usedPercent: Double) -> Int? {
        ...
        return thresholds
            .filter { used >= Double($0) }
            .max()
    }

    func handleUsageAlertTransition(provider: UsageProvider, snapshot: UsageSnapshot) {
        guard let window = self.usageAlertWindow(provider: provider, snapshot: snapshot) else {
            self.lastKnownUsageAlertThreshold.removeValue(forKey: provider)
            self.updateDockBadgeForUsageAlerts()
            return
        }
        let currentThreshold = self.highestCrossedUsageThreshold(usedPercent: window.usedPercent)
        let previousThreshold = self.lastKnownUsageAlertThreshold[provider]
        self.lastKnownUsageAlertThreshold[provider] = currentThreshold

        guard self.settings.usageAlertNotificationsEnabled else {
            self.updateDockBadgeForUsageAlerts()
            return
        }
        ...
        guard previousThreshold == nil || currentThreshold > previousThreshold! else {
            self.updateDockBadgeForUsageAlerts()
            return
        }

        let providerName = ProviderDescriptorRegistry.descriptor(for: provider).metadata.displayName
        let usedPercent = Int(max(0, min(100, window.usedPercent)).rounded())
        let title = "\(providerName) usage alert"
        let body = "\(usedPercent)% used (threshold \(currentThreshold)%)."
        AppNotifications.shared.post(
            idPrefix: "usage-threshold-\(provider.rawValue)-\(currentThreshold)",
            title: title,
            body: body,
            badge: NSNumber(value: currentThreshold))

        self.updateDockBadgeForUsageAlerts()
    }

    func handleResetWindowTransition(provider: UsageProvider, snapshot: UsageSnapshot) {
        guard let currentReset = snapshot.primary?.resetsAt ?? snapshot.secondary?.resetsAt else {
            self.lastKnownPrimaryResetAt.removeValue(forKey: provider)
            return
        }
        let previousReset = self.lastKnownPrimaryResetAt[provider]
        self.lastKnownPrimaryResetAt[provider] = currentReset
        guard self.settings.usageAlertNotificationsEnabled else { return }
        guard let previousReset else { return }
        guard currentReset != previousReset else { return }
        guard currentReset > previousReset else { return }
        guard previousReset <= Date().addingTimeInterval(120) else { return }

        let providerName = ProviderDescriptorRegistry.descriptor(for: provider).metadata.displayName
        AppNotifications.shared.post(
            idPrefix: "usage-reset-\(provider.rawValue)",
            title: "\(providerName) reset reached",
            body: "Best time to start a long session.",
            badge: NSNumber(value: 1))

        NSApp.dockTile.badgeLabel = "Reset"
        self.scheduleResetBadgeClear()
    }

    func updateDockBadgeForUsageAlerts() {
        let highestThreshold = self.lastKnownUsageAlertThreshold.values.max()
        if let highestThreshold {
            NSApp.dockTile.badgeLabel = "\(highestThreshold)%"
        } else if NSApp.dockTile.badgeLabel?.hasSuffix("%") == true {
            NSApp.dockTile.badgeLabel = nil
        }
    }

    private func scheduleResetBadgeClear() {
        self.dockBadgeClearTask?.cancel()
        self.dockBadgeClearTask = Task { @MainActor [weak self] in
            try? await Task.sleep(for: .seconds(8))
            guard let self else { return }
            if NSApp.dockTile.badgeLabel == "Reset" {
                self.updateDockBadgeForUsageAlerts()
            }
        }
    }
    ...
    func clearCostUsageCache() async -> String? {
        let errorMessage: String? = await Task.detached(priority: .utility) {
            ...
            let cacheDirs = [
                ...
                Self.costUsageAllTimeCacheDirectory(fileManager: fm),
            ]
