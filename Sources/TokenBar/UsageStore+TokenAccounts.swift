        ...
        fallbackSnapshot: UsageSnapshot?) async
    {
        ...
        case let .success(result):
            ...
            await MainActor.run {
                ...
                self.handleUsageAlertTransition(provider: provider, snapshot: labeled)
                self.handleResetWindowTransition(provider: provider, snapshot: labeled)
                self.snapshots[provider] = labeled
                self.recordTelemetry(provider: provider, snapshot: labeled)
