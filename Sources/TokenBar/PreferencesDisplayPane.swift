        ...
        ScrollView(.vertical, showsIndicators: true) {
            VStack(alignment: .leading, spacing: 16) {
                ...
                SettingsSection(contentSpacing: 12) {
                    ...
                    PreferenceToggleRow(
                        title: "Usage budget mode",
                        subtitle: "Set a target number of days and track over/under budget in real time.",
                        binding: self.$settings.usageBudgetModeEnabled)
                    HStack(alignment: .center, spacing: 12) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Budget target")
                                .font(.body)
                            Text("How many days your current quota should last.")
                                .font(.footnote)
                                .foregroundStyle(.tertiary)
                        }
                        Spacer()
                        Stepper(
                            value: self.$settings.usageBudgetTargetDays,
                            in: 1...30)
                        {
                            Text("\(self.settings.usageBudgetTargetDays) days")
                                .frame(minWidth: 72, alignment: .trailing)
                        }
                        .frame(maxWidth: 180, alignment: .trailing)
                    }
                    .disabled(!self.settings.usageBudgetModeEnabled)
                    .opacity(self.settings.usageBudgetModeEnabled ? 1 : 0.5)
                    PreferenceToggleRow(
                        title: "Threshold alerts",
                        subtitle: "Notify when usage crosses configured percentages and reflect alert state in menu bar.",
                        binding: self.$settings.usageAlertNotificationsEnabled)
                    HStack(alignment: .top, spacing: 12) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Alert thresholds")
                                .font(.body)
                            Text("Comma-separated percentages, e.g. 60,80,95.")
                                .font(.footnote)
                                .foregroundStyle(.tertiary)
                        }
                        Spacer()
                        TextField(
                            "60,80,95",
                            text: self.$settings.usageAlertThresholdsDisplay)
                            .textFieldStyle(.roundedBorder)
                            .multilineTextAlignment(.trailing)
                            .frame(maxWidth: 180)
                    }
                    .disabled(!self.settings.usageAlertNotificationsEnabled)
                    .opacity(self.settings.usageAlertNotificationsEnabled ? 1 : 0.5)
                    PreferenceToggleRow(
                        title: "Session tracking mode",
                        subtitle: "Estimate active prompting time from local usage changes and keep it on-device.",
                        binding: self.$settings.sessionTrackingModeEnabled)
                    HStack(alignment: .top, spacing: 12) {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Overview sorting")
                                .font(.body)
                            Text("How providers are ordered in the Overview tab.")
                                .font(.footnote)
                                .foregroundStyle(.tertiary)
                        }
                        Spacer()
                        Picker("Overview sorting", selection: self.$settings.overviewPrioritySortMode) {
                            ForEach(OverviewPrioritySortMode.allCases) { mode in
                                Text(mode.label).tag(mode)
                            }
                        }
                        .labelsHidden()
