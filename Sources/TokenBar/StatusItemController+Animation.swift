...
extension StatusItemController {
    ...
    private static let menuBarLogoSize = NSSize(width: 16, height: 16)
    ...
    func applyIcon(phase: Double?) {
        ...
        let statusIndicator: ProviderStatusIndicator = {
            var combined: ProviderStatusIndicator = .none
            for provider in self.store.enabledProviders() {
                let indicator = self.store.iconIndicator(for: provider)
                combined = ProviderStatusIndicator.maxSeverity(combined, indicator)
            }
            return combined
        }()
        let tintColor = self.menuBarTintColor(for: statusIndicator)
        let displayText = showBrandPercent
            ? self.menuBarDisplayText(for: primaryProvider, snapshot: snapshot)
            : nil

        if let logo = Self.menuBarLogoImage(statusIndicator: statusIndicator) {
            self.setButtonTintColor(nil, for: button)
            self.setButtonImage(logo, for: button)
        ...
        {
            self.setButtonTintColor(tintColor, for: button)
    ...
    func applyIcon(for provider: UsageProvider, phase: Double?) {
        ...
        let showBrandPercent = self.settings.menuBarShowsBrandIconWithPercent
        let indicator = self.store.iconIndicator(for: provider)
        let tintColor = self.menuBarTintColor(for: indicator)
        let displayText = showBrandPercent
            ? self.menuBarDisplayText(for: provider, snapshot: snapshot)
            : nil

        if let logo = Self.menuBarLogoImage(statusIndicator: indicator) {
            self.setButtonTintColor(nil, for: button)
            self.setButtonImage(logo, for: button)
            self.setButtonTitle(displayText, for: button)
            return
    ...
    private func menuBarTintColor(for indicator: ProviderStatusIndicator) -> NSColor? {
        switch indicator {
        case .none:
            return nil
        case .minor:
    ...
    private static func menuBarLogoImage(statusIndicator: ProviderStatusIndicator) -> NSImage? {
        _ = statusIndicator
        let size = Self.menuBarLogoSize
    ...
    private nonisolated static func drawBrandStatusOverlay(indicator: ProviderStatusIndicator, size: NSSize) {
        ...
        let color = Self.statusOverlayColor(for: indicator)
        switch indicator {
    ...
    private nonisolated static func statusOverlayColor(for indicator: ProviderStatusIndicator) -> NSColor {
        switch indicator {
        case .none:
            return .clear
        case .minor:
