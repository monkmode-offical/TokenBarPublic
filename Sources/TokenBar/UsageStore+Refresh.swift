    ...
    func refreshProvider(_ provider: UsageProvider, allowDisabled: Bool = false) async {
        ...
        case let .success(result):
            ...
            await MainActor.run {
                ...
                self.handleUsageAlertTransition(provider: provider, snapshot: scoped)
                self.handleResetWindowTransition(provider: provider, snapshot: scoped)
                self.snapshots[provider] = scoped
                self.recordTelemetry(provider: provider, snapshot: scoped)
