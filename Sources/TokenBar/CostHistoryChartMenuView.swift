...
struct CostHistoryChartMenuView: View {
    ...
    private let summaryLines: [String]
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 8) {
            ...
            if !self.summaryLines.isEmpty {
                VStack(alignment: .leading, spacing: 3) {
                    ForEach(Array(self.summaryLines.prefix(2).enumerated()), id: \.offset) { _, line in
                        Text(line)
                            .font(.caption)
                            .foregroundStyle(.secondary)
                            .lineLimit(1)
    ...
    private static let selectionBandColor = Color(nsColor: .labelColor).opacity(0.06)
    ...
    private func detailLines(model: Model) -> (primary: String, secondary: String?) {
        ...
        if let tokens = point.totalTokens {
            let primary = "\(dayLabel): \(cost) · \(UsageFormatter.tokenCountString(tokens))"
