...
struct SettingsStoreCoverageTests {
    ...
    @Test
    func telemetryDefaultsAndOverviewSortPersistAcrossReload() throws {
        let suite = "SettingsStoreCoverageTests-telemetry-overview-sort"
        let defaults = try #require(UserDefaults(suiteName: suite))
        defaults.removePersistentDomain(forName: suite)
        let configStore = testConfigStore(suiteName: suite)

        let first = Self.makeSettingsStore(userDefaults: defaults, configStore: configStore)
        #expect(first.sessionTrackingModeEnabled == true)
        #expect(first.overviewPrioritySortMode == .mostConstrained)

        first.sessionTrackingModeEnabled = false
        first.overviewPrioritySortMode = .bestAvailable

        let second = Self.makeSettingsStore(userDefaults: defaults, configStore: configStore)
        #expect(second.sessionTrackingModeEnabled == false)
        #expect(second.overviewPrioritySortMode == .bestAvailable)
    }
    ...
    @Test
    func licenseDefaultsAndServerURLNormalization() {
        let settings = Self.makeSettingsStore()
        #expect(settings.licenseState == .unlicensed)
        #expect(settings.licenseStatusMessage == LicenseState.unlicensed.defaultMessage)
        #expect(settings.licenseServerURL == "https://tokenbar.site")

        settings.licenseServerURL = "https://tokenbar.site/"
        #expect(settings.licenseServerURL == "https://tokenbar.site")
    }

    @Test
    func clearLicenseKeyResetsLicenseState() {
        let settings = Self.makeSettingsStore()
        settings.licenseKey = "TB-ABCD-EFGH-IJKL-MNOP"
        settings.licenseState = .active
        settings.licenseStatusMessage = "License active."
        settings.licenseLastVerifiedAt = Date()

        settings.clearLicenseKey()

        #expect(settings.licenseKey.isEmpty)
        #expect(settings.licenseState == .unlicensed)
        #expect(settings.licenseStatusMessage == LicenseState.unlicensed.defaultMessage)
        #expect(settings.licenseLastVerifiedAt == nil)
        #expect(settings.isLicenseUnlocked)
    }
