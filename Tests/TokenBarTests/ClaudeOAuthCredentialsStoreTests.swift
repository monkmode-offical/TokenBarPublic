...
struct ClaudeOAuthCredentialsStoreTests {
    ...
    func loadsFromKeychainCacheBeforeExpiredFile() throws {
        let service = "com.tokenbar.cache.tests.\(UUID().uuidString)"
    ...
    @Test
    func loadWithAutoRefresh_expiredTokenBarOwner_usesDirectRefreshPath() async throws {
        let service = "com.tokenbar.cache.tests.\(UUID().uuidString)"
    ...
    @Test
    func hasCachedCredentials_returnsFalseForExpiredUnrefreshableTokenBarCacheEntry() throws {
        KeychainCacheStore.setTestStoreForTesting(true)
        defer { KeychainCacheStore.setTestStoreForTesting(false) }
