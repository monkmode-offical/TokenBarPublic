...
struct MenuCardModelTests {
    ...
    func buildsMetricsUsingRemainingPercent() throws {
        ...
        #expect(model.metrics[0].windowLabel == "5h")
        #expect(model.metrics[1].windowLabel == "7d")
        #expect(model.metrics[1].resetText?.isEmpty == false)
    }

    @Test
    func codexModelFallsBackToDashboardWindowsWhenSnapshotWindowMissing() throws {
        let now = Date()
        let identity = ProviderIdentitySnapshot(
            providerID: .codex,
            accountEmail: "codex@example.com",
        ...
        #expect(model.metrics.count == 2)
        #expect(model.metrics[0].windowLabel == "5h")
        #expect(model.metrics[0].resetText?.isEmpty == false)
    ...
    func buildsMetricsUsingUsedPercentWhenEnabled() throws {
        ...
        #expect(model.metrics.first?.percentLabel.lowercased().contains("used") == true)
    ...
    func costSectionIncludesLast30DaysTokens() throws {
        ...
        let tokenSnapshot = CostUsageTokenSnapshot(
            ...
            allTimeTokens: 999,
            allTimeCostUSD: 123.45,
            allTimeSince: Date(timeIntervalSince1970: 1_700_000_000),
            daily: [],
            updatedAt: now)
        let model = UsageMenuCardView.Model.make(.init(
            provider: .codex,
        ...
        #expect(model.tokenUsage?.allTimeLine?.contains("All-time local logs") == true)
        #expect(model.tokenUsage?.allTimeLine?.contains("999") == true)
    }

    @Test
    func costSectionIncludesPlanSpendSummaryForCodexPro() throws {
        let now = Date(timeIntervalSince1970: 1_738_973_600) // Feb 1, 2025
        let metadata = try #require(ProviderDefaults.metadata[.codex])
        let snapshot = UsageSnapshot(
            primary: RateWindow(usedPercent: 0, windowMinutes: 300, resetsAt: nil, resetDescription: nil),
            secondary: nil,
        ...
        let tokenSnapshot = CostUsageTokenSnapshot(
            sessionTokens: 1024,
            sessionCostUSD: 4.2,
            last30DaysTokens: 96000,
            last30DaysCostUSD: 410,
            allTimeTokens: 250_000,
            allTimeCostUSD: 1120,
            allTimeSince: Date(timeIntervalSince1970: 1_735_776_000), // Dec 26, 2024
            daily: [
        ...
        #expect(model.tokenUsage?.planSpendLine?.contains("Subscription: Pro") == true)
        #expect(model.tokenUsage?.planSpendLine?.contains("$200.00/mo") == true)
        #expect(model.tokenUsage?.todayEconomicsLine?.contains("Today: Paid") == true)
        #expect(model.tokenUsage?.weekEconomicsLine?.contains("7d: Paid") == true)
        #expect(model.tokenUsage?.monthEconomicsLine?.contains("30d: Paid") == true)
        #expect(model.tokenUsage?.spendComparison?.title == "Today break-even")
        #expect(model.tokenUsage?.spendComparison?.paidUSD != nil)
        #expect(model.tokenUsage?.spendComparison?.valueUSD == 4.2)
    }
    ...
    func hidesEmailWhenPersonalInfoHidden() throws {
        ...
        #expect(model.creditsHintText?.contains("codex@example.com") != true)
    }
