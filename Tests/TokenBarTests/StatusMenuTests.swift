...
struct StatusMenuTests {
    ...
    @Test
    func updateBannerAppearsWhenUpdaterMarksReady() {
        self.disableMenuCardsForTesting()
        let settings = self.makeSettings()
        settings.statusChecksEnabled = false
        settings.refreshFrequency = .manual
        ...
        let fetcher = UsageFetcher()
        let store = UsageStore(fetcher: fetcher, browserDetection: BrowserDetection(cacheTTL: 0), settings: settings)
        let updater = TestUpdaterController()
        updater.updateStatus.isUpdateReady = true
        let controller = StatusItemController(
            store: store,
            settings: settings,
            account: fetcher.loadAccountInfo(),
        ...
        let titles = Set(menu.items.map(\.title))
        #expect(titles.contains("New update available - Update now"))
    }

    @Test
    func installUpdateActionUsesUpdaterInstallNow() {
        self.disableMenuCardsForTesting()
        let settings = self.makeSettings()
        settings.statusChecksEnabled = false
        settings.refreshFrequency = .manual
        ...
        #expect(updater.installUpdateNowCalls == 1)
        #expect(updater.checkForUpdatesCalls == 0)
    }
...
@MainActor
private final class TestUpdaterController: UpdaterProviding {
    ...
    private(set) var checkForUpdatesCalls = 0
    private(set) var installUpdateNowCalls = 0

    func checkForUpdates(_ sender: Any?) {
        self.checkForUpdatesCalls += 1
    }

    func installUpdateNow() {
        self.installUpdateNowCalls += 1
    }
}
